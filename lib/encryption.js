...
export async function encryptDocumentKey(documentKey, userKey) {
  ...
  return encryptContent(new TextDecoder().decode(keyData), userKey)
}

export async function decryptDocumentKey(encryptedDocumentKey, userKey) {
  const keyDataString = await decryptContent(encryptedDocumentKey, userKey)
  const keyData = new TextEncoder().encode(keyDataString)
